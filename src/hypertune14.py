...
with mlflow.start_run() as parent:    # to logs all child runs,.
    grid_search.fit(X_train, y_train)

    # log all the child runs
    for i in range(len(grid_search.cv_results_['params'])):

        with mlflow.start_run(nested=True) as child: # nested=true mean in mflow ui in exp we can see drop icon in that multiple exp 12
    ...
    train_df = mlflow.data.from_pandas(train_df) # Converting data (into mlflow format)
    ...
    test_df = mlflow.data.from_pandas(test_df)  # Converting data (into mlflow format)
